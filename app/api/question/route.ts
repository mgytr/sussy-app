...
export async function POST(request: NextRequest) {
    if (!imPaying()) return anyhow.blameSomeone(apiError.IM_BROKE, 503)
    ...
    try {
        const ai = new GoogleGenAI({ apiKey: getToken() });
        const tokens = await ai.models.countTokens({
            model: "gemini-3-flash-preview",
            contents: qn
        })

        if (!iStillCanPay(tokens.totalTokens || 0)) return anyhow.blameSomeone(apiError.IM_BROKE, 503)

        const response = anyhow.simulate
            ? {
        ...
        try {
            appendUsedTokens(response.usageMetadata?.totalTokenCount || 0)
            anyhow.record({ req: qn, res: parsedJson })
        } catch (err) { console.log("ono! cannot anyhow", err) }
